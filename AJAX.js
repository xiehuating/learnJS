...
xhr.readyState
	//异步请求时，需要判断该属性。该属性表示请求、响应过程的当前阶段。可取的值如下：
		//0：未初始化 UNSENT。尚未调用open()方法
		//1：启动 OPENED。已经调用open()方法，但尚未调用send()方法。
		//2：发送 HEADERS_REVEIVED。已经调用send()方法，但尚未接收到响应。
...
xhr.open(method,url,async);
	...
	//get请求：常用于向服务器查询某些信息
		...
		//查询字符串中的每个名称和值必须使用encodeURIComponent()进行编码，然后才能放到URL的末尾，见下函数：
...
xhr.onloadstart //在接收到响应的第一个字节时触发
xhr.onloadend //在通信完成或者触发error、abort或load事件后触发。没有浏览器支持
xhr.onload //重要。接收到完整的响应数据时触发。所有现代浏览器都支持。与xhr.onreadystatechange事件功能类似


xhr.onprogress //重要。在接收响应期间持续不断的触发
	//会接受一个event对象，
...
//超时事件
xhr.ontimeout //仅ie8+支持, xmlHttpResquest 2级
...
.append();//接收两个参数：键和值，对应表单字段的名字和字段中包含的值


xhr.timeout //表示请求在等待响应多少毫秒之后就终止, 仅ie8+支持。
xhr.ontimeout //timeout事件
